...
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
    ...
    const trailId = (req.query.trailId || (req.body && req.body.trailId));
    const queryTemplate = `[out:json];(way({id});>;way(bn););out;`
    const baseServiceUrl = "https://overpass-api.de/api/interpreter";

    if (!trailId) {
        context.res = {
            status: 400,
            body: "Please pass a trail id on the query string or in the request body"
        };
        return;
    }

    const query = queryTemplate.replace("{id}", trailId);
    ...
    await tryAndHandleError(
        ...
        async () => {
            ...
            let features = createTrailSegmentsFromOSM(trailId, response.data);
